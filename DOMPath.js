...
// and modified by Vittorio Cecchetto to make it work with puppeteer in browser
 ...
 * @return {string}
 */
DOMPath.fullQualifiedSelector = function(node, justSelector) {
    try {
...
DOMPath.cssPath = function(node, optimized) {
    ...
    while (contextNode) {
        const step = DOMPath._cssPathStep(contextNode, !!optimized, contextNode === node);
        if (!step)
 ...
 * @return {boolean}
 */
DOMPath.canGetJSPath = function(node) {
    let wp = node;
    while (wp) {
 ...
 * @return {string}
 */
DOMPath.jsPath = function(node, optimized) {
    if (node.nodeType !== NodeType.ELEMENT_NODE)
        return '';
 ...
 * @return {?DOMPath.Step}
 */
DOMPath._cssPathStep = function(node, optimized, isTargetNode) {
    if (node.nodeType !== NodeType.ELEMENT_NODE)
        return null;
    ...
    if (optimized) {
        if (id)
            return new DOMPath.Step(idSelector(id), true);
        const nodeNameLower = node.nodeName.toLowerCase();
        if (nodeNameLower === 'body' || nodeNameLower === 'head' || nodeNameLower === 'html')
            return new DOMPath.Step(nodeNameInCorrectCase(node), true);
    }
    const nodeName = nodeNameInCorrectCase(node);

    if (id)
        return new DOMPath.Step(nodeName + idSelector(id), true);
    const parent = node.parentNode;
    if (!parent || parent.nodeType === NodeType.DOCUMENT_NODE)
        return new DOMPath.Step(nodeName, true);
    ...
    return new DOMPath.Step(result, false);
};
 ...
 * @return {string}
 */
DOMPath.xPath = function(node, optimized) {
    ...
    let contextNode = node;
    while (contextNode) {
        const step = DOMPath._xPathValue(contextNode, optimized);
        if (!step)
            break; // Error - bail out early.
        steps.push(step);
        if (step.optimized)
 ...
 * @return {?DOMPath.Step}
 */
DOMPath._xPathValue = function(node, optimized) {
    let ownValue;
    const ownIndex = DOMPath._xPathIndex(node);
    if (ownIndex === -1)
        return null; // Error.
    ...
    switch (node.nodeType) {
        case NodeType.ELEMENT_NODE:
            if (optimized && node.getAttribute('id'))
                return new DOMPath.Step('//*[@id="' + node.getAttribute('id') + '"]', true);
    ...
    return new DOMPath.Step(ownValue, node.nodeType === NodeType.DOCUMENT_NODE);
};
 ...
 * @return {number}
 */
DOMPath._xPathIndex = function(node) {
